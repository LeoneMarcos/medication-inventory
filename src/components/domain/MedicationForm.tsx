...
import { isValidDateOnly, MAX_STOCK_LIMIT, parseDateInputToIso } from '../../lib/medications';
...
export function MedicationForm({ initialMedication, onSubmit, onCancel }: MedicationFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    quantity: initialMedication !== undefined ? String(initialMedication.quantity) : '',
    expirationDate: initialMedication?.expirationDate ?? '',
    manufacturer: initialMedication?.manufacturer ?? '',
    minimumStock: initialMedication !== undefined ? String(initialMedication.minimumStock) : '',
  });
  ...
  const [expirationInput, setExpirationInput] = useState(
    initialMedication?.expirationDate ? formatExpiration(initialMedication.expirationDate) : '',
  );
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (formData.quantity === '' || !Number.isInteger(qtyNum) || qtyNum < 0) {
      nextErrors.quantity = 'Enter a valid whole-number quantity (0 or greater)';
    ...
    if (formData.minimumStock === '' || !Number.isInteger(minNum) || minNum < 0) {
      nextErrors.minimumStock = 'Enter a valid whole-number minimum stock (0 or greater)';
    ...
    if (success === false) {
      setSubmitError('Storage failure: could not save medication to your browser storage.');
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="w-full">
        <label htmlFor="expirationDate" className="input-label">
          Expiration date<span className="required-mark" title="Required field">*</span>
        </label>
        <div className="relative">
          <Input
            ...
            onBlur={() => {
              if (formData.expirationDate && isValidDateOnly(formData.expirationDate)) {
