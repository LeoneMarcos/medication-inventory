...
describe('medication status (primary classification)', () => {
  it('identifies expired medication', () =>
    expect(getMedicationStatus(medication({ expirationDate: '2026-08-31' }), today)).toBe('expired'));
  it('identifies medication expiring within 30 days', () =>
    expect(getMedicationStatus(medication({ expirationDate: '2026-09-30' }), today)).toBe('expiring soon'));
  it('identifies healthy medication', () =>
    expect(getMedicationStatus(medication(), today)).toBe('healthy'));
  it('identifies low stock', () =>
    expect(getMedicationStatus(medication({ quantity: 5 }), today)).toBe('low stock'));
  it('prioritizes expiration over low stock for primary status', () =>
    expect(getMedicationStatus(medication({ quantity: 0, expirationDate: '2026-08-31' }), today)).toBe('expired'));
  it('prioritizes expiring soon over low stock for primary status when both apply', () =>
    expect(getMedicationStatus(medication({ quantity: 2, expirationDate: '2026-09-15' }), today)).toBe('expiring soon'));
});
...
describe('stock movements and limits', () => {
  ...
  it('rejects removal above available stock', () =>
    expect(() => removeStock(3, 4)).toThrow('Cannot remove more stock than is available.'));
  it('rejects non-integer, zero, or negative additions', () => {
    expect(() => addStock(10, 0)).toThrow('Stock amount must be a positive integer.');
    expect(() => addStock(10, -1)).toThrow('Stock amount must be a positive integer.');
    expect(() => addStock(10, 2.5)).toThrow('Stock amount must be a positive integer.');
  });
  it('rejects non-integer, zero, or negative removals', () => {
    expect(() => removeStock(10, 0)).toThrow('Stock amount must be a positive integer.');
    expect(() => removeStock(10, -2)).toThrow('Stock amount must be a positive integer.');
    expect(() => removeStock(10, 1.5)).toThrow('Stock amount must be a positive integer.');
  });
  it('enforces upper stock limit on additions using Number.MAX_SAFE_INTEGER', () => {
    ...
    expect(() => addStock(MAX_STOCK_LIMIT - 10, 11)).toThrow('Stock limit exceeded');
    expect(addStock(MAX_STOCK_LIMIT - 5, 5)).toBe(MAX_STOCK_LIMIT);
  });
  it('allows the minimum stock limit to be zero', () =>
    expect(getMedicationStatus(medication({ quantity: 0, minimumStock: 0 }))).toBe('low stock'));
});
