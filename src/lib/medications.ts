...
export function getMedicationFlags(
  ...
  const daysUntilExpiration = Math.round(
    (expiration.getTime() - currentDay.getTime()) / DAY_MS,
  );
...
export function addStock(currentQuantity: number, amount: number): number {
  ...
  if (amount > MAX_STOCK_LIMIT - currentQuantity) {
    throw new Error(
      `Stock limit exceeded: total quantity cannot exceed ${MAX_STOCK_LIMIT.toLocaleString("en-US")} units.`,
    );
  }
